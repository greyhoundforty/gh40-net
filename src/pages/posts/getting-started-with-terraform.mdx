...
With the changes made run `terraform plan` and inspect the output. Terrafirn compares the desired state, our updated main.tf fiile, with the current state.  In order for Terraform to reach the desired state it will destroy the existing container and create a new one based on the changes to the configuration.
